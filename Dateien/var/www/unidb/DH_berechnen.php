<?php
//eval ('$Wert = '.berechnen("jetzt",$Ausdruck).';');
//echo berechnen("jetzt",$Ausdruck);
function berechnen($Bezugszeitstempel,$Ausdruck) {
	...
	if ($Bezugszeitstempel == "jetzt"){
		//$Ausdruck = str_replace("AW(", "akt(", $Ausdruck);
	   $Bezugszeitpunkt = time();
	   $Bezugszeitstempel = strftime('%Y-%m-%d %H:%M:%S',$Bezugszeitpunkt);
	} else {
		if (floatval($Bezugszeitstempel) < 3000) {
			$Bezugszeitpunkt = strtotime($Bezugszeitstempel);
		} else {
			$Bezugszeitpunkt = floatval($Bezugszeitstempel);
			$Bezugszeitstempel = strftime('%Y-%m-%d %H:%M:%S',$Bezugszeitpunkt);
		}
	}
