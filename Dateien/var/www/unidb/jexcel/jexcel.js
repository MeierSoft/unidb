...
;(function (global, factory) {
        ...
        */
        ...
        obj.updateCornerPosition = function() {
            ...
            if (! obj.highlighted.length) {
                ...
                var lastY = last.getAttribute('data-y');
                ...
                if (obj.options.freezeRows) {
      		  		...
      		  		// Only check if the last row is not part of the merged cells
        ...
        obj.updateScroll = function(direction) {
            ...
            // Freeze rows?
            var freezed = obj.getFreezeHeight();
            // Top position check - TODO: change that to the bottom border of the element
            if (y > (obj.content.scrollTop + freezed) && y < (obj.content.scrollTop + h1)) {
                // In the viewport
            } else {
                // Out of viewport
                if (y < obj.content.scrollTop + 30) {
                    obj.content.scrollTop = y;
                    if (obj.content.scrollTop < 50) {
                        obj.content.scrollTop = 0;
                    }
                } else if (y < obj.content.scrollTop + freezed) {
                    obj.content.scrollTop = y - freezed - 1;
                } else {
                    obj.content.scrollTop = y - (h1 - 20);
                }
            }
        }
        ...
        obj.updateFreezePosition = function() {
            ...
            if (scrollTop > 0) {
                for (var i = 0; i < obj.options.freezeRows; i++) {
                    if (i > 0) {
                        // Must check if the previous column is hidden or not to determin whether the width shoule be added or not!
                        if (obj.rows[i-1].type !== "hidden") {
                            height += parseInt(obj.rows[i-1].clientHeight);
                        }
                    }
                    obj.rows[i].classList.add('jexcel_freezed');
                    obj.rows[i].style.top = '-1px';
                    for (var j = 0; j < obj.colgroup.length; j++) {
                        if (obj.colgroup[j] && obj.records[i][j]) {
                            var shifted = (scrollTop + (j > 0 ? obj.records[j][j-1].style.height : 0)) + 'px';
                            obj.records[i][j].classList.add('jexcel_freezed');
    ...
    jexcel.tabs = function(tabs, result) {
        ...
        if (! tabs.classList.contains('jexcel_tabs')) {
            ...
            var headers = tabs.children[1];
            var content = tabs.children[0];
        }
