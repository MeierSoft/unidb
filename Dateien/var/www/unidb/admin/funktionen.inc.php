...
{
	$_SESSION['DB_User']= strtolower($benutzer);
	$_SESSION['Hilfeliste'] = null;
	// Anweisung zusammenstellen
	$sql = "SELECT	`Password_extension`,`User_ID`, `Password` FROM	`User` WHERE LOWER(`UserName`) = '" .mysqli_real_escape_string($db, $benutzer). "' AND `activated` = 1;";
	...
	if (mysqli_num_rows($ergebnis) == 1) {
		...
		$sql = "SELECT
				...
				FROM
					`User`
				WHERE
					LOWER(`UserName`) = '" .mysqli_real_escape_string($db, $benutzer). "' AND
					`Password` = '" .mysqli_real_escape_string($db, $anmeldepw). "' AND
					`activated` = 1";
		// Anweisung an DB schicken
		$ergebnis = mysqli_query($db, $sql);
			...
			if ($angriff['mistrials'] != 0) {
				$sql = "UPDATE
						...
						SET
							`mistrials` = 0
						WHERE
							LOWER(`UserName`) = '" .mysqli_real_escape_string($db, $benutzer). "'
						LIMIT
							1";
				mysqli_query($db, $sql);
			}
...
function updateUser($benutzer, $db) {
	...
	$sql = "UPDATE
			...
			SET
				...
				`User_info` = '" .mysqli_real_escape_string($db, $_SERVER['HTTP_USER_AGENT']). "',
				`login` = '" .mysqli_real_escape_string($db, md5($_SERVER['REQUEST_TIME'])). "',
				`last_active` = NOW()
			WHERE
				LOWER(`UserName`) = '" .mysqli_real_escape_string($db, $benutzer). "'
			LIMIT
...
function checkUser( $db ) {
	...
	$sql = "SELECT	`Sprache`, `ip`, `User_info`, `login`, `Admin`, `Full_Name`, UNIX_TIMESTAMP(`last_active`) as last_active FROM `User` WHERE `UserName` = '" .mysqli_real_escape_string($db, $_SESSION['benutzername']). "' AND `activated` = 1";
	$ergebnis = mysqli_query($db, $sql);
	if (mysqli_num_rows( $ergebnis ) == 1)
	{
		...
		$_SESSION['admin'] = $benutzerdaten['Admin'];
		$_SESSION['TrendBreite'] = $benutzerdaten['TrendBreite'];
		$_SESSION['TrendHoehe'] = $benutzerdaten['TrendHoehe'];
...
//		if ($benutzerdaten['login'] != $_SESSION['anmeldung']) return false;
//		if (($benutzerdaten['last_active'] + 14400) <= $_SERVER['REQUEST_TIME']) return false;
	...
	// Wenn die Benutzerdaten in Ordnung sind 
	...
	$sql = "UPDATE
			...
			SET
				`last_active` = NOW()
			WHERE
				LOWER(`UserName`) = '" .mysqli_real_escape_string($db, $_SESSION['benutzername']). "'
			LIMIT
				1";
	mysqli_query($db, $sql);
	// Status zurückgeben 
...
function Berechtigung($Baum_ID, $Server_ID) {
	...
	mysqli_stmt_close($stmt);
	if($line1["owner"] != $_SESSION['User_ID']) {
		$query="SELECT `Path` FROM `User_Path` WHERE `User_ID` = ?;";
		$stmt = mysqli_prepare($db,$query);
...
function Kol_schreiben($query) {
	...
	if($_SESSION['V_Daten'] == null || $_SESSION['V_Daten'] == NULL) {
...
function uKol_schreiben($Start,$query,$Typen,$Ar) {
	...
	$Tabelle = "";
	$query1_gepuffert = 0;
//	$_SESSION['uV_Daten'] = null;
	if($_SESSION['uV_Daten'] == null || $_SESSION['uV_Daten'] == NULL) {
	...
	for ($i=0;$i<count($_SESSION['uV_Daten']);$i++){
		if($_SESSION['uV_Daten'][$i]["Database"] > "") {
				...
				if(strtolower(substr($query,0,6)) == "update") {
					if($Tabelle == "") {
						...
						if($Tabelle == "Baum" and strpos($query, "geloescht") == 0 and $Start == 1) {
							...
							for($x=0; $x < count($dynFeld); $x++) {
								$query1 = $query1."'".$dynFeld[$x]."', '".htmlentities($line2[$dynFeld[$x]])."', ";
							}
			...
			$Ar = json_encode($Ar);
			$SQL = "INSERT INTO `Puffer` (`Server`, `SQL_Text`, `Typen`, `Ar`) VALUES ('".$_SESSION['uV_Daten'][$i]["IP"]."', '".$query."', '".$Typen."', '".$Ar."');";
			$stmt = mysqli_prepare($db, $SQL);
			mysqli_stmt_execute($stmt);
			$SQL = "INSERT INTO `Eingangspuffer` (`Server`, `SQL_Text`, `Typen`, `Ar`) VALUES ('".$_SESSION['uV_Daten'][$i]["IP"]."', '".$query."', '".$Typen."', '".$Ar."');";
			$stmt = mysqli_prepare($db, $SQL);
			mysqli_stmt_execute($stmt);
