    ...
    $.fn.ip_Modal = function (options) {
        ...
        var buttons = {
            ...
            cancel: { text: 'CANCEL', style: "background: #9c3b3b;background: -moz-linear-gradient(top,  #9c3b3b 0%, #752c2c 100%); background: -webkit-gradient(linear, left top, left bottom, color-stop(0%,#9c3b3b), color-stop(100%,#752c2c)); background: -webkit-linear-gradient(top,  #9c3b3b 0%,#752c2c 100%); background: -o-linear-gradient(top,  #9c3b3b 0%,#752c2c 100%); background: -ms-linear-gradient(top,  #9c3b3b 0%,#752c2c 100%); background: linear-gradient(to bottom,  #9c3b3b 0%,#752c2c 100%); filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#9c3b3b', endColorstr='#752c2c',GradientType=0 );", onClick: function () { $().ip_Modal({ show: false }) } },
				hide: { text: 'HIDE', style: "background: #9c3b3b;background: -moz-linear-gradient(top,  #9c3b3b 0%, #752c2c 100%); background: -webkit-gradient(linear, left top, left bottom, color-stop(0%,#9c3b3b), color-stop(100%,#752c2c)); background: -webkit-linear-gradient(top,  #9c3b3b 0%,#752c2c 100%); background: -o-linear-gradient(top,  #9c3b3b 0%,#752c2c 100%); background: -ms-linear-gradient(top,  #9c3b3b 0%,#752c2c 100%); background: linear-gradient(to bottom,  #9c3b3b 0%,#752c2c 100%); filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#9c3b3b', endColorstr='#752c2c',GradientType=0 );", onClick: function () { $().ip_Modal({ show: false }) } }
        };


                       
        var control = $('#ip_modal');
        try{if (options.show == false){document.getElementById("ip_modal").parentElement.removeChild(document.getElementById("ip_modal"));}} catch (err) {}
...
function ip_SetupFx(GridID) {
    ...
    $('#' + GridID).ip_AddFormula({ formulaName: 'akt_Wert', functionName: 'akt_Wert', tip: 'Gibt den aktuellen Wert eines Tags aus.', inputs: '(Tag,Zeitstempel,Einheit,Richtung)', example: '<br/>T1,1,1,"senkrecht"'});
    $('#' + GridID).ip_AddFormula({ formulaName: 'AWerte', functionName: 'Archivwerte', tip: 'Gibt die Archivwerte für den angegebenen Zeitraum des betreffenden Tags aus.', inputs: '(Tag,Zeitraum von,Zeitraum bis,Zeitstempel,Einheit,vt,vt_interpol,Unixzeit,Typ,Richtung)', example: '<br/>T1,2019-10-10 12:00:00,2019-10-11 12:00:00,1,1,1,1,1,"senkrecht"'});
    $('#' + GridID).ip_AddFormula({ formulaName: 'AW', functionName: 'Archivwert', tip: 'Gibt den archivierten Wert für den angegebenen Zeitpunkt des betreffenden Tags aus.', inputs: '(Tag,Zeitpunkt,Zeitstempel,interpoliert,Einheit,Typ,Richtung)', example: '<br/>T1,2019-10-10 12:00:00,"Wert davor",1,1,"Rohwert","senkrecht"'});
    $('#' + GridID).ip_AddFormula({ formulaName: 'MW', functionName: 'Mittelwert', tip: 'Gibt den Mittelwert für den angegebenen Zeitraum des betreffenden Tags aus.', inputs: '(Tag,Zeitraum von,Zeitraum bis,Einheit,step,Richtung)', example: '<br/>T1,2019-10-10 12:00:00,2019-10-11 12:00:00,1,1,"senkrecht"'});
    $('#' + GridID).ip_AddFormula({ formulaName: 'iW', functionName: 'iWert', tip: 'Gibt den interpolierten Wert für den angegebenen Zeitpunkt des betreffenden Tags aus.', inputs: '(Tag,Zeitpunkt,Einheit,Richtung)', example: '<br/>T1,2019-10-10 12:00:00,1,"senkrecht"'});
}
...
function ip_ShowEditToolHelpToolTip(GridID, fxInfo) {
    ...
    var fxToolTip = '';
	 if (fxToolTip == undefined) {$().ip_Modal({ show:false, fade:100 });}
    if (fxInfo != null) {
...
function ip_fxCalculate(GridID, fxString, row, col) {
    ...
    try {
        ...
        fxString = fxString.replace(ip_GridProps['index'].regEx.range, function (arg) { if (arg.indexOf(":") > -1){return 'ip_fxRangeObject("' + GridID + '",' + row + ',' + col + ',"' + arg + '")';} else {return 'ip_fxRange("' + GridID + '",' + row + ',' + col + ',"' + arg + '")';}});
...
function Range_Typ(arg,Text1){
	if (arg.indexOf(':') > -1){
		var Text = 'ip_fxRangeObject';
	} else {
		var Text = 'ip_fxRange';
	}
	return Text + Text1 + arg + '")';
}
