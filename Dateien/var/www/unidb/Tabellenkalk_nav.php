...
<?php
		...
		echo '<div id="schaltfl_2" style="width: 20%;" onclick="umschalten(2);" class="schaltfl_mobil">'.$Text[112].'</div>';
...
<?php
	if($anzeigen == 1) {
		if($Zeitstempel == "") {
			echo "<td>".$Text[40].": <input type='text' name='Bezeichnung' value='".$line["Bezeichnung"]."'></td>";
			echo '<td><input type="submit" name="Aktion", id="speichern_schalter" value="'.$Text[108].'" onclick="speichern();"></td>';
			echo "<td><a href='verschieben.php?Baum_ID=".$Baum_ID."&Server_ID=".$Server_ID."'>".$Text[4]."</a></td><td><a href='./kopieren.php?original=".$Baum_ID."&Server_ID=".$Server_ID."' target='_self'>".$Text[5]."</a></td><td><a href='loeschen.php?Baum_ID=".$Baum_ID."&Server_ID=".$Server_ID."' target='Hauptrahmen'>".$Text[3]."</a></td>";
		} else {
			echo "<td><input type='hidden' name='Bezeichnung' value='".$line["Bezeichnung"]."'></td>";
		}
		if($geloescht == 1) {
   ...
   $abfrage = "SELECT `Hist_ID`, `Timestamp` FROM `Baumhistorie` WHERE `Baum_ID` = ? AND `Server_ID` = ? ORDER BY `Timestamp` DESC;";
	...
	if(mysqli_num_rows($result1) > 0) {
		echo "<td>".$Text[149].":&nbsp;<select name='Timestamp1' id='timestamp1' onchange='Vers_zeigen();' value='".$Zeitstempel."'><option></option>";
